...
struct OnboardingFlow: View {
    ...
    var body: some View {
        ZStack {
            switch step {
            case 0: SplashScreen(onNext: { step = 1 })
            case 1: WhatIsOrcaScreen(onNext: { step = 2 })
            case 2: ThoughtBubblesScreen(onNext: { step = 3 })
            case 3: FirstDropScreen(
                        onNext: { step = 4 },
                        onMemorySaved: { text in firstMemoryText = text }
                    )
            case 4: NotificationsScreen(onNext: { step = 5 }, memoryText: firstMemoryText)
            case 5: AllSetScreen(onDone: { hasCompletedOnboarding = true })
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 8) {
                    Text("You're all set")
                        .font(.custom("DMSans-Medium", size: 32))
                        .foregroundColor(.white)
                    Text("The more you Drop, the smarter Orca gets.")
                        .font(.custom("DMSans-Regular", size: 16))
                        .foregroundColor(.white.opacity(0.5))
                        .multilineTextAlignment(.center)
                        .padding(.horizontal, 32)
